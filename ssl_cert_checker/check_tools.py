...
def check_certificates(endpoints, warning_days=WARN_IF_DAYS_LESS_THAN):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=WORKER_THREAD_COUNT) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                err_count += 1
                messages.append('{} ERROR {}'.format(format_host_port(host, port).ljust(max_host_port_len), ex))
            else:
                ...
                messages.append('{} {:<5} expires in {}'.format(
                    format_host_port(host, port).ljust(max_host_port_len),
                    'WARN' if days_remaining < warning_days else 'OK',
                    time_remaining_txt))
